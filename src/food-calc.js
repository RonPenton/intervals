...
const chosenFood = 'gummyWorms';
async function go() {
    console.log('Fetching rides from Intervals.icu...');
    const rawRides = await (0, intervals_api_1.getRides)(seasonStart);
    const pzone = training_definitions_1.CogganPowerZones.find(z => z.value === zone);
    const seconds = minHours * 60 * 60;
    const rides = rawRides.filter(ride => ride.moving_time >= seconds &&
        ride.icu_intensity >= pzone.minPowerPct &&
        ride.icu_intensity < pzone.maxPowerPct).map(ride => {
        const hours = ride.moving_time / 3600;
        const caloriesPerHour = ride.calories / hours;
        const carbPercentage = (0, training_1.computeRoughCarbBurnedPercentage)(ride.icu_intensity / 100);
    ...
    const m = (meters) => (meters / 1609.34).toFixed(0);
