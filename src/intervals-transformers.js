...
exports.pruneActivityFields = pruneActivityFields;
exports.pruneWellnessFields = pruneWellnessFields;
const training_1 = require("./training");
function pruneActivityFields(record, zones, intervalLengths) {
    const { icu_ftp, icu_training_load, icu_joules, icu_weighted_avg_watts, distance, moving_time, total_elevation_gain, average_speed, calories, average_temp, start_date_local, icu_intensity, icu_atl, icu_ctl } = record;
    ...
    const metersToMiles = (meters) => (meters / 1609.34).toFixed(0);
    const metersToFeet = (meters) => (meters * 3.28084).toFixed(0);
    const mpsToMph = (metersPerSecond) => (metersPerSecond * 2.23694).toFixed(1);
    ...
    return {
        ...
        zone: (0, training_1.getZoneForRide)(record, zones, intervalLengths)
    };
}
function pruneWellnessFields(record) {
    const { id, atl, atlLoad, ctl, ctlLoad, rampRate } = record;
    return {
        date: id,
